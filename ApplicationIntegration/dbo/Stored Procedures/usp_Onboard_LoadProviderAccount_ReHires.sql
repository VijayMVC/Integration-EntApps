...
GO
/****** Object:  StoredProcedure [dbo].[usp_Onboard_LoadProviderAccount_ReHires]    Script Date: 10/26/2016 13:05:12 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER Procedure [dbo].[usp_Onboard_LoadProviderAccount_ReHires]
...
--****8/1/2014 disabled RC piece will activate after troubleshooting *****
	...
	FROM (
		SELECT RS.ProviderMasterID, PMB.SFID, 'RH' AS Flag
			...
			JOIN ProviderMaster PM
				ON RS.ProviderMasterID = PM.ProviderMasterID
	...
	UNION
		SELECT DISTINCT Sc.Provider_Master_ID__c AS ProviderMasterID, SC.Id AS SFID, 'RC' As Flag 
				...
				WHERE SS.Next_Recredentialed__c =  Convert(Datetime, GETDATE()) + 137
				AND SS.Record_Status__c NOT LIKE 'Former%'
				AND SC.Provider_Master_ID__c IS NOT NULL
				AND SC.Provider_Master_ID__c NOT IN (SELECT ProviderMasterID
										FROM AppStaging.dbo.OnboardProviderAccountForSFDC
										WHERE ProviderType_Flag = 'RC'
										AND CreDate >= GETDATE() - 30 )
		) T
		
DROP TABLE 	RecredApps	
SELECT * INTO RecredApps
FROM #TempPA
WHERE Flag = 'RC'

--email Recred apps being loaded for any issues

DECLARE @thequery nvarchar(max) = 'SELECT * FROM ApplicationIntegration.dbo.RecredApps WHERE Flag = ''RC''';

PRINT @thequery

EXEC msdb.dbo.sp_send_dbmail
		@recipients= 'singhr@medamerica.com',
		@copy_recipients = 'RafaelR@medamerica.com',
		@query = @thequery,
		@subject=N'Recred Provider Notification',
    		@body=N'Hi,

	Attached are the Recred providers being loaded	',

		@attach_query_result_as_file = 1 ;


--ProviderAccount_Re for Recred and Rehires
...
TRUNCATE TABLE ProviderAccount_Re
	...
	SELECT PM.ProviderMasterID,
		...
		SC.MobilePhone,
		PMB.PhoneNbr1,
		LEFT(LTRIM(RTRIM(PMB.PhoneNbr2)),20) PhoneNbr2,
		PMB.FaxNbr1,
		PMB.CEPEmail,
		PMB.EmailAddress,
		NULL,
...
-- Work History *** Part 1 - Excluding Gap in Work History****
	...
	ORDER BY PMB.ProviderMasterID, To__c desc
...
--		a. State License = Medical License
	...
	SELECT SC.Name,
		...
		Sc.Issuer_1__c,
		'State',
		LEFT(SC.Cert_Lic_Number__c, 20) Cert_Lic_Number__c,
	...
	ORDER BY PMB.ProviderMasterID, SC.DEA_State_of_Registration__c
...
--** Special case - when CDS State DEA does not exist in SFDC then need to insert a 'N/A' for Onboarding data
	...
	INSERT INTO ProviderLicenses
	SELECT DISTINCT NULL AS SFDCName, PMB.ProviderMasterID, PMB.WebID, '', 
		'CDS State DEA', 'N/A', '', '', '', '', '', ''
	FROM #TempPA
	JOIN ProviderMasterBase PMB 
		ON PMB.ProviderMasterID = #TempPA.ProviderMasterID
	WHERE #TempPA.ProviderMasterID NOT IN (SELECT ProviderMasterID
...
*/
	...
	ORDER BY PMB.ProviderMasterID, SC.Expiration_Date__c desc 
