...
CREATE PROCEDURE [dbo].[usp_LoadPMUpdateArchive_New] 
AS
...
/* Round1 - Load providers that are inactive but still have site
		They are transferred so that the Sites can be removed*/
		...
		SELECT StaffGroup1, StaffGroup2, ProviderMasterID, 
			...
			WHERE ProviderMasterID IN (SELECT ProviderMasterID FROM SiteBase)	
			AND EXISTS (SELECT 1 
