...
11-17-2008:
  ...
  12/3/20101 - Reetika singh - Modified Cross Joins 

*/
CREATE PROCEDURE [dbo].[LoadCurrentWebEarningsPAClient_YearEnd]
...
--		BEGIN TRANSACTION;
				...
				TRUNCATE TABLE YTDEarningsStatementPA	
				TRUNCATE TABLE YTDEarningsStatementStagingPA

				-- LOAD CATEGORY E
				INSERT INTO YTDEarningsStatementStagingPA (CsuFlxIDeb, ProviderMasterID, CheckDate, CheckNo,
						...
						LEFT JOIN	dbo.PRTIME PRT ON EMP.EMPLOYEE = PRT.EMPLOYEE
							...
							JOIN	(SELECT Y.COMPANY, Y.EMPLOYEE, Y.PER_END_DATE, Y.CHECK_DATE, Y.CHECK_TYPE, Y.CHECK_NBR, 
									 ...
									 FROM (SELECT PYM.COMPANY, PYM.EMPLOYEE, PYM.PER_END_DATE, PYM.CHECK_DATE, PYM.CHECK_TYPE
											...
											WHERE YEAR(PYM.CHECK_DATE) = '2015' AND PYM.CHECK_NBR <> 0) Y )TEMP
									ON TEMP.EMPLOYEE = EMP.EMPLOYEE AND TEMP.COMPANY = EMP.COMPANY
									AND TEMP.CHECK_ID = PRT.CHECK_ID		
						WHERE YEAR(TEMP.CHECK_DATE) = '2015'	
				...
				UPDATE YTDEarningsStatementStagingPA
				...
				INSERT INTO YTDEarningsStatementStagingPA (CsuFlxIDeb, ProviderMasterID, CheckDate,CheckNo,
						...
						WHERE YEAR(PYM.CHECK_DATE) = '2015' 
							AND EMP.PAY_FREQUENCY BETWEEN @PAY_FREQUENCY_START AND @PAY_FREQUENCY_END 
				...
				INSERT INTO YTDEarningsStatementStagingPA (CsuFlxIDeb, ProviderMasterID, PeriodEnding, CheckDate, 
				...
				SELECT DISTINCT CsuFlxIDeb, ProviderMasterID
				...
				SELECT DISTINCT M2.CsuFlxIDeb, M2.ProviderMasterID
				...
				INTO TempTable13
				FROM TempTable11 S
				LEFT JOIN TempTable12 M2
						ON	M2.CsuFlxIDeb = S.CsuFlxIDeb
				...
				INSERT INTO YTDEarningsStatementStagingPA (CsuFlxIDeb, ProviderMasterID, PeriodEnding, CheckDate, 
							...
							SELECT C.CsuFlxIDeb
								...
								FROM (SELECT DISTINCT B.CsuFlxIDeb, B.ProviderMasterID
									  ...
									  FROM TempTable11 AS A
									  CROSS JOIN TempTable13 AS B
				...
				FROM YTDEarningsStatementStagingPA PA
				JOIN (SELECT A.CsuFlxIDeb 
					  ...
					  FROM (SELECT A.CsuFlxIDeb 
							...
							FROM YTDEarningsStatementStagingPA AS A
							LEFT JOIN (SELECT t1.CategoryFlag
										...
										FROM dbo.YTDEarningsStatementStagingPA t1
				...
				LEFT JOIN	dbo.PAYDEDUCTN PYD ON EMP.EMPLOYEE = PYD.EMPLOYEE
					...
					WHERE YEAR(T_TAX.CHECK_DATE) = '2015' 
					AND EMP.PAY_FREQUENCY BETWEEN @PAY_FREQUENCY_START AND @PAY_FREQUENCY_END 
				...
				SELECT DISTINCT M2.CsuFlxIDeb, M2.ProviderMasterID
							...
							, M2.CalendarYear, M2.CsdFlxID
				INTO TempTable23
				FROM TempTable21 AS F
				LEFT JOIN TempTable22 AS M2
						ON	M2.CsuFlxIDeb = F.CsuFlxIDeb
				...
				INSERT INTO YTDEarningsStatementStagingPA (CsuFlxIDeb, ProviderMasterID, CheckDate, CheckNo,
							...
							SELECT C.CsuFlxIDeb
								...
								FROM (SELECT DISTINCT B.CsuFlxIDeb, B.ProviderMasterID
											...
											, B.CalendarYear, B.CsdFlxID			
									  FROM TempTable21 AS A
									  CROSS JOIN TempTable23 AS B
									  WHERE A.CsuFlxIDeb = B.CsuFlxIDeb 
									  AND A.CheckDate <= B.CheckDate									  
									  ) C
							  LEFT JOIN 
									(SELECT CsuFlxIDeb, ProviderMasterID, CheckDate, CheckNo, PeriodEnding,
											LastName,FirstName,MiddleName,FullName, LbrCustPayCode,
											CsxShortDesc,CalendarYear
									 FROM TempTable21) PA
								ON  C.CsuFlxIDeb = PA.CsuFlxIDeb
								AND C.CheckDate = PA.CheckDate
								AND C.CheckNo = PA.CheckNo
				...
				*/
				...
				INSERT INTO YTDEarningsStatementStagingPA (CsuFlxIDeb, ProviderMasterID, CheckDate,CheckNo,
					...
					WHERE YEAR(T_TAX1.CHECK_DATE) = '2015' 
					AND EMP.PAY_FREQUENCY BETWEEN @PAY_FREQUENCY_START AND @PAY_FREQUENCY_END 
					AND (PYD.PROCESS_LEVEL BETWEEN @PL_START1 AND @PL_END1
					OR PYD.PROCESS_LEVEL BETWEEN @PL_START2 AND @PL_END2)					
				...
				SELECT DISTINCT M2.CsuFlxIDeb, M2.ProviderMasterID
						...
						, M2.CalendarYear, M2.CsdFlxID	
				INTO TempTable33	
				FROM TempTable31 AS G
				LEFT JOIN TempTable32 AS M2
							ON	M2.CsuFlxIDeb = G.CsuFlxIDeb
				...
				INSERT INTO YTDEarningsStatementStagingPA (CsuFlxIDeb, ProviderMasterID, CheckDate,CheckNo,
							...
							SELECT C.CsuFlxIDeb
								...
								FROM (SELECT DISTINCT B.CsuFlxIDeb, B.ProviderMasterID
											...
											, B.CalendarYear, B.CsdFlxID			
									  FROM TempTable31 AS A
									  CROSS JOIN TempTable33 B
									  	WHERE A.CsuFlxIDeb = B.CsuFlxIDeb 
									  		AND A.CheckDate <= B.CheckDate									  
									  ) C
							  LEFT JOIN 
				...
				INSERT INTO YTDEarningsStatementStagingPA (CsuFlxIDeb, ProviderMasterID, CheckDate, CheckNo,
				   ...
				   LEFT JOIN	dbo.PAYDEDUCTN PYD ON EMP.EMPLOYEE = PYD.EMPLOYEE
						...
						WHERE YEAR(TMP.CHECK_DATE) = '2015' 			
						AND EMP.PAY_FREQUENCY BETWEEN @PAY_FREQUENCY_START AND @PAY_FREQUENCY_END 
						AND (PYD.PROCESS_LEVEL BETWEEN @PL_START1 AND @PL_END1
						OR PYD.PROCESS_LEVEL BETWEEN @PL_START2 AND @PL_END2)	 					
				...
				INSERT INTO YTDEarningsStatementStagingPA (CsuFlxIDeb, CsdFlxID, LbrCustPayCode, ProviderMasterID, CheckDate, CheckNo, PeriodEnding, 
					...
					FROM YTDEarningsStatementStagingPA ST
						INNER JOIN (SELECT DISTINCT PYD.EMPLOYEE, PYD.DED_CODE, DED.CHECK_DESC
											   ...
											   LEFT JOIN dbo.PAYMASTR PYM ON PYM.EMPLOYEE = PYD.EMPLOYEE
													...
													WHERE (DED.CALC_TYPE <> 'T'
														...
														AND DED.TAX_STATUS BETWEEN '01' AND '12' 
														OR DED.TAX_STATUS = 'X'
														AND YEAR(PYM.CHECK_DATE) = '2015' ) --AND PYD.EMPLOYEE = 2069
													) TMP
...
--							AND TMP.DED_CODE = ST.LbrCustPayCode
				...
				-- As per Carol Long 2/18/2014 - 
				--UPDATE CssFlxIDCsu = 0 FOR SALARIED EMPLOYEES WHERE RATE = 0(Not Adding SHFT_DIFF_RATE)
				UPDATE PA
				SET CssFlxIDCsu = 0
				FROM YTDEarningsStatementStagingPA PA
				WHERE PA.CategoryFlag = 'K_TMP'
				AND HourlyRate = 0
				AND PA.CsuFlxIDeb IN (SELECT EMPLOYEE FROM EMPLOYEE
									WHERE SALARY_CLASS = 'S')
				...
				*/
				...
				DELETE FROM YTDEarningsStatementStagingPA
				WHERE CategoryFlag = 'K_TMP'
				...
				SELECT DISTINCT M2.CsuFlxIDeb, M2.ProviderMasterID
					...
					, M2.CalendarYear, M2.CsdFlxID
				INTO TempTable43
				FROM TempTable41 AS K
				LEFT JOIN TempTable42 M2
						ON	M2.CsuFlxIDeb = K.CsuFlxIDeb
				...
				INSERT INTO YTDEarningsStatementStagingPA (CsuFlxIDeb, ProviderMasterID, CheckDate,CheckNo,
							...
							SELECT C.CsuFlxIDeb
								...
								FROM (SELECT DISTINCT B.CsuFlxIDeb, B.ProviderMasterID 
									  ...
									  FROM TempTable41 AS A
									  CROSS JOIN TempTable43 AS B
									  WHERE A.CsuFlxIDeb = B.CsuFlxIDeb 
									  AND A.CheckDate <= B.CheckDate													   
									  ) C
							  LEFT JOIN 
				...
				SELECT DISTINCT L.CsuFlxIDeb, M2.ProviderMasterID
					, M2.CheckDate, M2.CheckNo, M2.PeriodEnding
					, M2.LastName,M2.FirstName,M2.MiddleName,M2.FullName										
					, M2.CalendarYear, M2.CsdFlxID
				INTO TempTable53
				FROM TempTable51 AS L
				LEFT JOIN TempTable52 M2
						ON	M2.CsuFlxIDeb = L.CsuFlxIDeb
				...
				INSERT INTO YTDEarningsStatementStagingPA (CsuFlxIDeb, ProviderMasterID, CheckDate,CheckNo,
							...
							SELECT C.CsuFlxIDeb
								...
								FROM (SELECT DISTINCT A.CsuFlxIDeb, B.ProviderMasterID 
											...
											, B.LastName, B.FirstName, B.MiddleName, B.FullName	
											, B.CalendarYear, B.CsdFlxID
									  FROM TempTable51 AS A
										CROSS JOIN TempTable53 AS B
									  WHERE A.CsuFlxIDeb = B.CsuFlxIDeb 
									  AND A.CheckDate <= B.CheckDate													   
									  ) C
							  LEFT JOIN 
...
--									, SumOfCurrentHours = ISNULL(SUM(PRT.HOURS),0)
				...
				SELECT DISTINCT P.CsuFlxIDeb, M2.ProviderMasterID
					, M2.CheckDate, M2.CheckNo, M2.PeriodEnding
					, M2.LastName,M2.FirstName,M2.MiddleName,M2.FullName										
					, M2.CalendarYear
				INTO TempTable63
				FROM TempTable61 AS P
				LEFT JOIN TempTable62 AS M2
						ON	M2.CsuFlxIDeb = P.CsuFlxIDeb
				...
				INSERT INTO YTDEarningsStatementStagingPA (CsuFlxIDeb, ProviderMasterID, CheckDate, CheckNo,
							...
							SELECT C.CsuFlxIDeb
								...
								FROM (SELECT DISTINCT A.CsuFlxIDeb, B.ProviderMasterID 
											...
											, B.LastName, B.FirstName, B.MiddleName, B.FullName	
											, B.CalendarYear
									  FROM TempTable61 AS A
										CROSS JOIN TempTable63 AS B
									  WHERE A.CsuFlxIDeb = B.CsuFlxIDeb 
									  AND A.CheckDate <= B.CheckDate													   
									  ) C
							  LEFT JOIN 
...
AdjustmentFlagM*/
INSERT INTO YTDEarningsStatementStagingPA (CsuFlxIDeb, ProviderMasterID, CheckDate, CheckNo, PeriodEnding, 
...
---------------------------------------END MedAmerica addition 2/19/09
			...
			END
					...
					INSERT INTO YTDEarningsStatementPA(CsuFlxIDeb, ProviderMasterID, CheckDate, CheckNo,
					...
					INSERT INTO YTDEarningsStatementPA (CsuFlxIDeb, ProviderMasterID, PeriodEnding, CheckDate, 
...
/*					SELECT MIN(CsuFlxIDeb) AS CsuFlxIDeb, MIN(CsdFlxID) AS CsdFlxID, MIN(ProviderMasterID) AS ProviderMasterID, MIN(CheckDate) AS CheckDate, CheckNo, MIN(PeriodEnding) AS PeriodEnding, 
					...
					INTO #Temp
					FROM YTDEarningsStatementPA
					WHERE (PayCode IN ('HSA','DP') OR CategoryFlag = 'S')
					GROUP BY CheckNo

					DELETE YTDEarningsStatementPA
					WHERE CheckNo IN (SELECT CheckNo FROM #Temp)
							AND CategoryFlag = 'S'

					INSERT INTO YTDEarningsStatementPA (CsuFlxIDeb, CsdFlxID, ProviderMasterID, CheckDate, CheckNo, PeriodEnding, 
									LastName, FirstName, MiddleName, FullName, CalendarYear, 
...
*/
					...
					INSERT INTO YTDEarningsStatementArchivePA
						SELECT * FROM YTDEarningsStatementPA
