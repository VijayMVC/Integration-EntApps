...
-- =============================================
CREATE PROCEDURE [dbo].[LoadVisaDepartmentSummary_YearEnd]
...
BEGIN
	...
	DECLARE		@ListOfVisa	TABLE(CsuFlxIDeb int)
	DECLARE		@ListOfOldVisa		TABLE(CsuFlxIDeb int)
	...
	IF (@PeriodStartDate IS NULL)
		BEGIN
			SET @PeriodStartDate = (SELECT MAX(RangeEndDate) FROM VisaSummaryTransaction WHERE CreateDate < '12/30/2015')
		END

	IF (@PeriodEndDate IS NULL)
		BEGIN
			SET @PeriodEndDate = '12/31/2015'
			SET @PEndDefault = 1
		END


	SET @CreateDate = '12/31/2015'
...
********************************************************************************************************************************/
	...
	IF @err <>  0
		BEGIN
			...
			VALUES ('OS', '12/31/2015', 'Error @ position 1') 
			
		END

	INSERT INTO @ListOfVisa (CsuFlxIDeb)
		SELECT DISTINCT EMPLOYEE 
		FROM PROD.dbo.PAEMPLOYEE
		WHERE HIRE_SOURCE = 'VISA'
	...
	IF @err <>  0
		BEGIN
			...
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', '12/31/2015', 'Error @ position 2') 
	...
	TRUNCATE TABLE VisaSummaryTransactionStaging
	...
	*******************************************************************************************************/
	...
	---CheckDates inbtw	
	INSERT INTO dbo.VisaSummaryTransactionStaging (CheckDate, CheckNo, PayPeriod, Dept, DeptDescr, 
		...
		WHERE CheckDate > @PeriodStartDate AND CheckDate <= @PeriodEndDate AND CategoryFlag = 'K' 
			  AND CsuFlxIDeb IN (SELECT CsuFlxIDeb FROM @ListOfVisa) 
	...
	IF @err <>  0
		BEGIN
			...
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', '12/31/2015', 'Error @ position 3') 
...
--			PRINT 'Error @ position 4'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', '12/31/2015', 'Error @ position 4') 
...
--			PRINT 'Error @ position 5'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', '12/31/2015', 'Error @ position 5') 
			
		END
-----------------------------------------------------------------------------------------------------------
INSERT INTO @ListOfOldVisa (CsuFlxIDeb)
	...
	(
		...
		SELECT DISTINCT CsuFlxIDeb, Dept
		FROM VisaSummaryTransactionStaging
	...
	WHERE B.CsuFlxIDeb IS  NULL AND A.CsuFlxIDeb IN 
		(
		SELECT CsuFlxIDeb FROM @ListOfVisa
		)
...
--			PRINT 'Error @ position 6'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', '12/31/2015', 'Error @ position 6') 
				...
				FROM
					...
					FROM 
						(SELECT CsuFlxIDeb, CheckDate, CheckNo, CsdFlxID
						 FROM YTDEarningsStatementStagingPA
						 WHERE CsuFlxIDeb in (SELECT CsuFlxIDeb FROM @ListOfOldVisa) and CategoryFlag = 'K'
					...
					(SELECT CsuFlxIDeb MaxCD_CsuFlxIDeb, Max(CheckDate) as MaxCD_CheckDate
						 FROM YTDEarningsStatementStagingPA
						WHERE CsuFlxIDeb in (SELECT CsuFlxIDeb FROM @ListOfOldVisa) and CategoryFlag = 'K' and CheckDate < @PeriodEndDate
...
--			PRINT 'Error @ position 7'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', '12/31/2015', 'Error @ position 7') 
			
		END
-----------------------------------------------------------------------------------------------------------
	---Update RangeStartDate, RangeEndDate 
	UPDATE VisaSummaryTransactionStaging
	...
	DELETE VisaSummaryTransactionStaging
...
--			PRINT 'Error @ position 8'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', '12/31/2015', 'Error @ position 8') 
...
-------------------------------------------------------------------------------------------------
	...
	SELECT @C= COUNT(*) 
		FROM VisaSummaryTransactionStaging 
		WHERE CheckNo 
			IN (SELECT DISTINCT CheckNo FROM VisaSummaryTransaction)
--	IF (@PayRollCheckDate IN (SELECT DISTINCT PayRollCheckDate FROM VisaSummaryTransaction))

	---Append this PayRollCheckDate into the Transaction table from the staging table.
	INSERT INTO dbo.VisaSummaryTransaction ( CheckDate, CheckNo, PayPeriod, Dept, DeptDescr, 
						CsuFlxIDeb, ProviderMasterID, LastName, FirstName, 
						MiddleName, FullName, PayCode, PayCodeDescr, TotalCurrentHours, 
...
--	IF (@PayRollCheckDate IN (SELECT DISTINCT PayRollCheckDate FROM VisaSummarySubtotals))
...
--			PRINT 'Error @ position 11'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', '12/31/2015', 'Error @ position 11') 
...
--			PRINT 'Error @ position 12'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', '12/31/2015', 'Error @ position 12') 
			
		END

---Now its time to group together the checkdates
INSERT INTO dbo.VisaSummarySubtotals (PayRollCheckDate,
					PayRollPayPeriod,
...
--			PRINT 'Error @ position 13'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', '12/31/2015', 'Error @ position 13') 
...
--			PRINT 'Error @ position 14'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', '12/31/2015', 'Error @ position 14') 
...
--			PRINT 'Error @ position 15'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', '12/31/2015', 'Error @ position 15') 
...
--			PRINT 'Error @ position 16'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', '12/31/2015', 'Error @ position 16.') 
			
		END

	INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage, PayRollCheckDate, RangeStartDate, RangeEndDate)
	VALUES ('OS', '12/31/2015', 'Looks like it made it to the end', @PayRollCheckDate, @PeriodStartDate, @PeriodEndDate ) 
END
