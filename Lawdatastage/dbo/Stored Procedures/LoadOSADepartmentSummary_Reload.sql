...
-- =============================================
CREATE PROCEDURE [dbo].[LoadOSADepartmentSummary_Reload]
...
BEGIN
	...
	DECLARE		@ListOfOSA	TABLE(CsuFlxIDeb int)
	DECLARE		@ListOfOldOSA		TABLE(CsuFlxIDeb int)
	...
	IF (@PeriodStartDate IS NULL)
		BEGIN
			SET @PeriodStartDate = (SELECT MAX(RangeEndDate) FROM OSASummaryTransactionRe 
									WHERE CreateDate < (SELECT MAX(Credate)-1
														FROM YTDEarningsStatementSemiMonthly))
		END

	IF (@PeriodEndDate IS NULL)
		BEGIN
			SET @PeriodEndDate = (SELECT MAX(Credate) FROM YTDEarningsStatementSemiMonthly)
			SET @PEndDefault = 1
		END


	SET @CreateDate = (SELECT MAX(Credate) FROM YTDEarningsStatementSemiMonthly)
	SET @CalendarYear = (SELECT YEAR(MAX(Credate)) FROM YTDEarningsStatementSemiMonthly)
...
This portion is no longer needed because of the new date logic that is being implemented --- 11/17/09 JS
	...
	--way no checks are missed on 1/1. No 12/31 cchecks will be picked b/c the YTDEarningsStatementSemiMonthly table won't have any 
...
********************************************************************************************************************************/
	...
	IF @err <>  0
		BEGIN
			...
			VALUES ('OS', getdate(), 'Error @ position 1') 
			
		END

	INSERT INTO @ListOfOSA (CsuFlxIDeb)
		SELECT DISTINCT EMPLOYEE 
		FROM PROD.dbo.PAEMPLOYEE
		WHERE HIRE_SOURCE LIKE '%OSA-CEP%'
	...
	IF @err <>  0
		BEGIN
			...
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', getdate(), 'Error @ position 2') 
	...
	TRUNCATE TABLE OSASummaryTransactionStagingRe
	...
	*******************************************************************************************************/
	...
	---CheckDates inbtw	
	INSERT INTO dbo.OSASummaryTransactionStagingRe (CheckDate, CheckNo, PayPeriod, Dept, DeptDescr, 
		...
		WHERE CheckDate > @PeriodStartDate AND CheckDate <= @PeriodEndDate AND CategoryFlag = 'K' 
			  AND CsuFlxIDeb IN (SELECT CsuFlxIDeb FROM @ListOfOSA) 
	...
	IF @err <>  0
		BEGIN
			...
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', getdate(), 'Error @ position 3') 
...
--			PRINT 'Error @ position 4'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', getdate(), 'Error @ position 4') 
...
--			PRINT 'Error @ position 5'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', getdate(), 'Error @ position 5') 
			
		END
-----------------------------------------------------------------------------------------------------------
INSERT INTO @ListOfOldOSA (CsuFlxIDeb)
	...
	(
		...
		SELECT DISTINCT CsuFlxIDeb, Dept
		FROM OSASummaryTransactionStagingRe
	...
	WHERE B.CsuFlxIDeb IS  NULL AND A.CsuFlxIDeb IN 
		(
		SELECT CsuFlxIDeb FROM @ListOfOSA
		)
...
--			PRINT 'Error @ position 6'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', getdate(), 'Error @ position 6') 
				...
				FROM
					...
					FROM 
						(SELECT CsuFlxIDeb, CheckDate, CheckNo, CsdFlxID
						 FROM YTDEarningsStatementSemiMonthly
						 WHERE CsuFlxIDeb in (SELECT CsuFlxIDeb FROM @ListOfOldOSA) and CategoryFlag = 'K'
					...
					(SELECT CsuFlxIDeb MaxCD_CsuFlxIDeb, Max(CheckDate) as MaxCD_CheckDate
						 FROM YTDEarningsStatementSemiMonthly
						WHERE CsuFlxIDeb in (SELECT CsuFlxIDeb FROM @ListOfOldOSA) and CategoryFlag = 'K' and CheckDate < @PeriodEndDate
...
--			PRINT 'Error @ position 7'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', getdate(), 'Error @ position 7') 
			
		END
-----------------------------------------------------------------------------------------------------------
	---Update RangeStartDate, RangeEndDate 
	UPDATE OSASummaryTransactionStagingRe
	...
	DELETE OSASummaryTransactionStagingRe
...
--			PRINT 'Error @ position 8'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', getdate(), 'Error @ position 8') 
...
-------------------------------------------------------------------------------------------------
	...
	SELECT @C= COUNT(*) 
		FROM OSASummaryTransactionStagingRe
		WHERE CheckNo 
			IN (SELECT DISTINCT CheckNo FROM OSASummaryTransactionRe)
--	IF (@PayRollCheckDate IN (SELECT DISTINCT PayRollCheckDate FROM OSASummaryTransaction))

	---Append this PayRollCheckDate into the Transaction table from the staging table.
	INSERT INTO dbo.OSASummaryTransactionRe ( CheckDate, CheckNo, PayPeriod, Dept, DeptDescr, 
						CsuFlxIDeb, ProviderMasterID, LastName, FirstName, 
						MiddleName, FullName, PayCode, PayCodeDescr, TotalCurrentHours, 
...
--	IF (@PayRollCheckDate IN (SELECT DISTINCT PayRollCheckDate FROM OSASummarySubtotals))
...
--			PRINT 'Error @ position 11'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', getdate(), 'Error @ position 11') 
			
		END

INSERT INTO #CheckDateRolledUp(	CheckDate, CheckNo,
				CsuFlxIDeb, ProviderMasterID, LastName, FirstName,
				MiddleName, FullName,
	...
	SELECT X.CheckDate AS CheckDate, X.CheckNo,
...
--			PRINT 'Error @ position 12'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', getdate(), 'Error @ position 12') 
			
		END

---Now its time to group together the checkdates
INSERT INTO OSASummarySubtotalsRe(PayRollCheckDate,
					PayRollPayPeriod,
	...
	DECLARE @Temp int
	SET @Temp = (SELECT COUNT(*) FROM OSASummarySubtotalsRe WHERE CreateDate >= '3/1/2012')
...
--			PRINT 'Error @ position 13'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', getdate(), 'Error @ position 13') 
...
--			PRINT 'Error @ position 14'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', getdate(), 'Error @ position 14') 
...
--			PRINT 'Error @ position 15'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', getdate(), 'Error @ position 15') 
...
--			PRINT 'Error @ position 16'
			INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage)
			VALUES ('OS', getdate(), 'Error @ position 16.') 
			
		END

	INSERT INTO ErrorMsg (SPName, ErrorDate, ErrorMessage, PayRollCheckDate, RangeStartDate, RangeEndDate)
	VALUES ('OS', getdate(), 'Looks like it made it to the end', @PayRollCheckDate, @PeriodStartDate, @PeriodEndDate ) 
END
