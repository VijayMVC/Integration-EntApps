...
-- Author: Jay Santhan
-- Create date: 9/9/11
-- Description:	Email Adoption Report for Galen population, non compliance edition.  Calculates the percent of providers 
--				who have not accessed their email within the last four weeks.
-- Last Modified: 9/9/12
-- =============================================
CREATE PROCEDURE [dbo].[Galen_LoadEmailAdoptionPercent_NonCompliance] 
...
BEGIN
	...
	IF (@ParamRunDate IS NULL)
		BEGIN
			SET @CurrDate = (SELECT TOP 1 TS_scriptRun 
							 FROM dbo.SSO_Email_Count_Log
							 ORDER BY TS_scriptRun DESC) + 1;
		END
	ELSE 
		BEGIN
	...
	SET @AlreadyLoaded = (SELECT 1 
						WHERE EXISTS (SELECT 1 
									  FROM Galen_EmailAdoption_NonCompliance
	...
	BEGIN
		...
		TRUNCATE TABLE Galen_NoAdoptionProviders;
		...
		WHERE Status = 0 AND StaffGroup2 = 'GALEN' AND Site1 <> '';
		...
		WHILE @WeeksBackCounter <> 0
			BEGIN 
				...
				WHERE TS_scriptRun >= @ScriptDate AND
					  NOT EXISTS (SELECT 1 
								  ...
								  WHERE SSOP.ProviderMasterID = SC.ProviderMasterID) 
							--	  AND
					  --EXISTS (SELECT 1 
							--	  FROM #ProvidersWithHours AS PWH
							--	  WHERE SSOP.ProviderMasterID = PWH.ProviderMasterID)
				...
				SET @MDCount = (SELECT Count(*)
								FROM #ProviderMasterBaseActive as PWH --#ProvidersWithHours AS PWH
								WHERE PWH.Ttl = 'MD');
				...
				-----------------------------------------------
				--** INSERT INTO Galen_EmailAdoption_NC_Count TABLE **--
				-----------------------------------------------
				INSERT INTO Galen_EmailAdoption_NC_Count
				(RunDate, WeeksBack, 
				 ClickCountMD, NoClickReadCountMD, OnlyASUCountMD, MDCount, 
				 ClickCountPA, NoClickReadCountPA, OnlyASUCountPA, PACount)
				VALUES
				(@RunDate, @WeeksBackCounter,
				 @ClickCountMD, @NoClickReadCountMD, @OnlyASUCountMD, @MDCount, 
				 @ClickCountPA, @NoClickReadCountPA, @OnlyASUCountPA, @PACount)
				
				------------------------------------------------
				--** INSERT INTO Galen_NoAdoptionProviders TABLE **--
				------------------------------------------------
				--Inserts the list of providers that did not access their cep email, added on 12/7/11
				
				INSERT INTO Galen_NoAdoptionProviders (Ttl, ProviderMasterID, WebID, WeeksBack, RunDate, 
				...
				----------------------------------------------------------------------------------------
				--** Create a detailed listing of active providers and whether they check ther email**--
				----------------------------------------------------------------------------------------
				-- Insert initial list of all active providers
				IF (@WeeksBackCounter = 4)
					BEGIN
						INSERT INTO Galen_EmailAdoption_Detail (Ttl, SiteCode, SiteName, ProviderMasterID, WebID, 
						LastName, FirstName, Title, RunDate)
							SELECT PMBA.Ttl, PMBA.Site1, HR.HospitalName, PMBA.ProviderMasterID, PMBA.WebID, 
								   PMBA.LastName, PMBA.FirstName, PMBA.Title, @RunDate
							FROM #ProviderMasterBaseActive AS PMBA
							LEFT JOIN ApplicationIntegration.dbo.Hospital_region AS HR
							ON PMBA.Site1 = HR.Code
					END	
				-- Update Galen_EmailAdoption_Detail table with the values for each week as you go thru
				-- while loop
				IF (@WeeksBackCounter = 1)
					BEGIN
						-- Set those providers who did not check their email
						UPDATE Galen_EmailAdoption_Detail
						SET OneWk = -1
						WHERE EXISTS 		
								(SELECT 1 FROM dbo.Galen_NoAdoptionProviders AS NA
								 WHERE NA.WeeksBack = 1 
									   AND Galen_EmailAdoption_Detail.ProviderMasterID = NA.ProviderMasterID)
							  AND Galen_EmailAdoption_Detail.RunDate = @RunDate
						
						-- Set those providers who did not work in the time period in question
						UPDATE Galen_EmailAdoption_Detail
						SET OneWk = 0
						WHERE NOT EXISTS
								(SELECT 1 FROM #ProvidersWithHours AS NoHr
								 WHERE Galen_EmailAdoption_Detail.ProviderMasterID = NoHr.ProviderMasterID)
							  AND Galen_EmailAdoption_Detail.RunDate = @RunDate
							  
							  
						-- Set those providers who checked their email
						UPDATE Galen_EmailAdoption_Detail
						SET OneWk = 1
						WHERE 					 
								(EXISTS (SELECT 1
									  FROM #SSO_Click AS SSO
									  WHERE Galen_EmailAdoption_Detail.ProviderMasterID = SSO.ProviderMasterID)
					  OR EXISTS (SELECT 1
									  FROM #NoClickReadFlag AS NCRF
									  WHERE Galen_EmailAdoption_Detail.ProviderMasterID = NCRF.ProviderMasterID AND
											NCRF.ReadFlag = 1)
					  OR EXISTS (SELECT 1
									  FROM #OnlyActiveSync AS ASU
									  WHERE Galen_EmailAdoption_Detail.ProviderMasterID = ASU.MPID))
				...
				---------------------------------------------------------------------------
				-- Overall Site counts
				INSERT INTO Galen_NCSiteCount (WeeksBack, RunDate, Region, SiteCode, SiteName, TotalNCProviders)
					SELECT @WeeksBackCounter, @RunDate, HR.Region, Site1, HR.HospitalName, count(Site1) AS NumInSite 
					FROM (
							SELECT Site1 
							FROM Galen_NoAdoptionProviders
				...
				FROM Galen_NCSiteCount AS NCMD
					 JOIN 
				...
				FROM Galen_NCSiteCount AS NCPA
					 JOIN 
			...
			INSERT INTO Galen_NCRegionCount (WeeksBack, RunDate, Region, RegionName, TotalNCProviders, MDNCProviders, PANCProviders)
		...
		----------------------------------------------------------------------
		--** TRANSPOSE AND INSERT INTO Galen_EmailAdoption_NonCompliance TABLE **--
		----------------------------------------------------------------------
		-- Reformat data for the Galen_EmailAdoption_NonCompliance table. This table is formated like this
		-- because Excel charts can read this better
		INSERT INTO Galen_EmailAdoption_NonCompliance
